...
class _PdfCompressScreenState extends State<PdfCompressScreen> {
  ...
  late final String _originalSizeMb;
  ...
  void initState() {
    ...
    _originalSizeMb = _formatBytes(bytes);
  }
  ...
  Future<bool> _onWillPop() async {
    // Sonuç üretilmiş ama kullanıcı vazgeçip geri çıktıysa geçici dosyayı temizle
    if (_result != null) {
      await TempFileManager.deleteFile(_result!.compressedFile);
    }
    return true;
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: (didPop, result) async {
        if (didPop && _result != null && result == null) {
          await TempFileManager.deleteFile(_result!.compressedFile);
        }
      },
      child: Scaffold(
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Mevcut Boyut: $_originalSizeMb MB',
                  ...
                  return Container(
                    ...
                    child: RadioListTile<CompressionLevel>(
                      ...
                      title: Text(
                        lvl.name == 'medium'
                            ? 'Önerilen Sıkıştırma'
                            : (lvl.name == 'low' ? 'Hafif Sıkıştırma' : 'Yüksek Sıkıştırma'),
                        ...
                        children: [
                          Column(
                            children: [
                              ...
                              const SizedBox(height: 4),
                              Text('$_originalSizeMb MB', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                            ],
                          ),
